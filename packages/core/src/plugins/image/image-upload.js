...
import { runUploadHandler } from './image-upload-handler.js';
import {
  maxFileSize, formatMB, serverErrorDetail, fileSizeError,
  maxDataUriSize, dataUriSizeError,
} from './image-upload-limits.js';

// Re-exported so existing importers of image-upload.js are unaffected by the
// split — the limits moved for file-length reasons, not API reasons.
export {
  maxFileSize, formatMB, serverErrorDetail, fileSizeError,
  maxDataUriSize, dataUriSizeError,
};
...
// Exported so the T12 handler module reuses the exact same measurement (and
// its stall timeout) rather than growing a second, subtly different copy.
export function measureImage(src, doc) {
    ...
    json = await new Promise((resolve, reject) => {
      ...
      xhr.addEventListener('load', () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          // Surface the server's own error message when it sent one (a short
          // text/JSON body), so the user sees WHY — not just a bare status code.
          reject(new Error(`Image upload failed: ${serverErrorDetail(xhr)}`));
        }
      });
      ...
      // IMG14: a hung server would otherwise leave the progress bar/spinner stuck
      // forever. Fail after a timeout (config.imageUploadTimeout ms, default 60s;
      // 0 disables) so the user gets an actionable error instead of an infinite wait.
      const timeoutMs = typeof config.imageUploadTimeout === 'number'
        ? config.imageUploadTimeout : 60000;
      if (timeoutMs > 0) {
        xhr.timeout = timeoutMs;
        xhr.addEventListener('timeout', () => reject(
          new Error(`Image upload timed out after ${Math.round(timeoutMs / 1000)}s — the server did not respond.`)));
      }
...
export async function processImageFile(file, config = {}, onProgress = null, signal = null, doc = document) {
  ...
  // A custom handler wins over imageUploadUrl when both are set (T12): the
  // handler is the more specific, deliberately-written instruction, and
  // silently preferring the URL would make a configured handler look broken.
  if (typeof config.imageUploadHandler === 'function') {
    return runUploadHandler(file, config, onProgress, signal, doc);
  }

  if (config.imageUploadUrl) {
    return uploadFile(file, config.imageUploadUrl, onProgress, signal, doc, config);
  }
  // IMG11: enforce the inline data-URI cap before encoding (the raw fileSizeError
  // guards the upload/general size; this guards the inline-bloat path specifically).
  const dataErr = dataUriSizeError(file, config);
